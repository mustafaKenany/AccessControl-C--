...
public static class DatabaseMigrator
{
    public static void EnsureSchemaUpToDate(AppDbContext db)
    {
        ...
        var migrations = new[]
        {
            ...
            // v4.6.18: mirror the cloud API key into the DB so a reinstall (DB kept) recovers
            // the SAME key instead of generating a new one that the cloud rejects.
            @"IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('AppSettings') AND name = 'CloudApiKey')
              ALTER TABLE AppSettings ADD CloudApiKey nvarchar(100) NOT NULL DEFAULT '';",
